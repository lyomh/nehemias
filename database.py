...
DATABASE_URL = os.getenv("DATABASE_URL")  # Para PostgreSQL en la nube
SQLITE_PATH = os.getenv("DATABASE_URL_SQLITE", "nehemias.db")
...
def traducir_sql(query):
    ...
    if "INSERT OR IGNORE INTO subregiones" in query:
        query = "INSERT INTO subregiones (nombre) VALUES (%s) ON CONFLICT (nombre) DO NOTHING"
    elif "INSERT OR IGNORE INTO catalog_eventos" in query:
        query = "INSERT INTO catalog_eventos (nombre) VALUES (%s) ON CONFLICT (nombre) DO NOTHING"
...
def obtener_catalogo_eventos():
    with obtener_conexion() as conn:
        return pd.read_sql_query(traducir_sql("SELECT * FROM catalog_eventos ORDER BY nombre"), conn)

def obtener_subregiones():
    with obtener_conexion() as conn:
        return pd.read_sql_query(traducir_sql("SELECT * FROM subregiones ORDER BY nombre"), conn)

# --- Gestión de Usuarios ---

def obtener_usuarios_por_creador(id_creador, filtro_rol=None):
    with obtener_conexion() as conn:
        query = "SELECT u.*, s.nombre as subregion_nombre FROM users u LEFT JOIN subregiones s ON u.subregion_id = s.id WHERE u.created_by = ?"
        params = [id_creador]
        if filtro_rol:
            query += " AND u.role = ?"
            params.append(filtro_rol)
        return pd.read_sql_query(traducir_sql(query), conn, params=params)

def crear_usuario(nombre_usuario, hash_clave, rol, id_subregion=None, id_evento=None, creado_por=None):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "INSERT INTO users (username, password_hash, role, subregion_id, evento_id, created_by) VALUES (?, ?, ?, ?, ?, ?)"
            cursor.execute(
                traducir_sql(query),
...
def obtener_actividades_filtradas(rol_usuario, id_usuario, id_subregion=None, estados=None, ano=None, mes=None):
    with obtener_conexion() as conn:
        ...
        if estados:
            ...
            query += f" AND a.status IN ({marcadores})"
...
def crear_actividad(titulo, descripcion, id_subregion, creado_por):
    with obtener_conexion() as conn:
        ...
        try:
            ahora = datetime.now()
            query = "INSERT INTO actividades (titulo, descripcion, subregion_id, created_by, fecha_asignacion) VALUES (?, ?, ?, ?, ?)"
            cursor.execute(
                traducir_sql(query),
            ...
            if IS_POSTGRES:
                ...
                id_actividad = list(cursor.fetchone().values())[0] if IS_POSTGRES else cursor.lastrowid
            else:
...
def tomar_actividad(id_actividad, id_usuario):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE actividades SET status = 'En Proceso', fecha_toma = ? WHERE id = ? AND status = 'Activa'"
            cursor.execute(
                traducir_sql(query),
...
def responder_actividad(id_actividad, respuesta, id_usuario):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE actividades SET status = 'En Revisión', respuesta_tecnica = ?, fecha_respuesta = ? WHERE id = ? AND status = 'En Proceso'"
            cursor.execute(
                traducir_sql(query),
...
def cerrar_actividad(id_actividad, id_usuario):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE actividades SET status = 'Cerrada', fecha_cierre = ? WHERE id = ? AND status = 'En Revisión'"
            cursor.execute(
                traducir_sql(query),
                (datetime.now(), id_actividad)
            )
...
def anular_actividad(id_actividad, motivo, id_usuario):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE actividades SET status = 'Anulada', anulacion_motivo = ? WHERE id = ?"
            cursor.execute(
                traducir_sql(query),
                (motivo, id_actividad)
            )
...
def obtener_ajustes_por_actividad(id_actividad):
    with obtener_conexion() as conn:
        return pd.read_sql_query(traducir_sql("SELECT * FROM ajustes WHERE actividad_id = ?"), conn, params=[id_actividad])

def guardar_gestion_granular(ajustes_df):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE ajustes SET resultado_rs = ?, nota_tecnica = ?, evidencia_rs = ? WHERE id = ?"
...
def guardar_validacion_red(ajustes_df):
    with obtener_conexion() as conn:
        cursor = conn.cursor()
        try:
            query = "UPDATE ajustes SET validacion_red = ? WHERE id = ?"
            for _, f in ajustes_df.iterrows():
                cursor.execute(
                    traducir_sql(query),
...
def obtener_conteos_estado(rol_usuario, id_usuario=None, ano=None, mes=None, id_subregion=None):
    with obtener_conexion() as conn:
        query = "SELECT status, COUNT(*) as total FROM actividades WHERE 1=1"
        params = []
        if rol_usuario == 'RS':
            query += " AND subregion_id = ?"
            params.append(id_subregion)
...
def obtener_resumen_ajustes(ano=None, mes=None, id_subregion=None):
    with obtener_conexion() as conn:
        ...
        if mes:
            query += " AND strftime('%m', a.created_at) = ?"
            params.append(str(mes).zfill(2))
        query += " GROUP BY resultado_rs, validacion_red"
        return pd.read_sql_query(traducir_sql(query), conn, params=params)
...
def obtener_productividad_eventos(ano=None, mes=None, id_subregion=None):
    with obtener_conexion() as conn:
        ...
        if mes:
            query += " AND strftime('%m', a.created_at) = ?"
            params.append(str(mes).zfill(2))
        query += " GROUP BY nom_eve ORDER BY total DESC LIMIT 10"
        return pd.read_sql_query(traducir_sql(query), conn, params=params)
...
def obtener_anos_disponibles():
    with obtener_conexion() as conn:
        query = "SELECT DISTINCT strftime('%Y', created_at) as year FROM actividades ORDER BY year DESC"
        df = pd.read_sql_query(traducir_sql(query), conn)
...
def obtener_tendencia_mensual(ano, id_subregion=None):
    ...
    with obtener_conexion() as conn:
        ...
        if id_subregion:
            query += " AND a.subregion_id = ?"
            params.append(id_subregion)
        query += " GROUP BY mes_num ORDER BY mes_num"
        df_real = pd.read_sql_query(traducir_sql(query), conn, params=params)
...
def eliminar_evento(id_evento):
    with obtener_conexion() as conn:
        try:
            conn.execute(traducir_sql("DELETE FROM catalog_eventos WHERE id = ?"), (id_evento,))
            conn.commit()
...
def alternar_estado_usuario(id_usuario, estado_actual):
    ...
    with obtener_conexion() as conn:
        try:
            conn.execute(traducir_sql("UPDATE users SET is_active = ? WHERE id = ?"), (nuevo_estado, id_usuario))
            conn.commit()
...
def agregar_evento(nombre):
    with obtener_conexion() as conn:
        try:
            conn.execute(traducir_sql("INSERT INTO catalog_eventos (nombre) VALUES (?)"), (nombre,))
            conn.commit()
...
def actualizar_evento(id_evento, nombre):
    with obtener_conexion() as conn:
        try:
            conn.execute(traducir_sql("UPDATE catalog_eventos SET nombre = ? WHERE id = ?"), (nombre, id_evento))
            conn.commit()
...
def restablecer_clave_usuario(id_usuario, nuevo_hash_clave):
    with obtener_conexion() as conn:
        try:
            conn.execute(traducir_sql("UPDATE users SET password_hash = ? WHERE id = ?"), (nuevo_hash_clave, id_usuario))
            conn.commit()
